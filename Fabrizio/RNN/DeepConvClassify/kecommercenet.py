...
class Ecommercenet:
    ...
    def build(width, height, depth, classes, mul_factor, weightsPath=None):
        ...
        model.add(Convolution2D(92, mul_factor*21, mul_factor*21, border_mode="same", input_shape=(depth, height, width)))
        model.add(Activation(conv_activation))
		
        # second set of CONV => RELU => POOL
        model.add(Convolution2D(72, mul_factor*3, mul_factor*3, border_mode="same"))
        model.add(Activation(conv_activation))
